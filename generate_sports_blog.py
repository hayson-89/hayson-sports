...
def get_google_news():
    """Google News RSS에서 최신 스포츠 뉴스 수집"""
    news_items = []
    feeds = [
...
def classify_news(news_items):
    ...
    news_keywords = ["경기", "승리", "패배", "골", "득점", "우승", "선수", "리그",
                     "축구", "야구", "농구", "배구", "골프", "테니스", "UFC", "올림픽"]
    info_keywords = ["방법", "가이드", "팁", "기초", "훈련", "운동법", "기술", "비법"]
    notice_keywords = ["대회", "일정", "신청", "행사", "개막", "폐막", "개최", "참가"]

    for item in news_items:
        title = item["title"]
        if any(k in title for k in info_keywords):
            classified["스포츠정보"].append(title)
        elif any(k in title for k in notice_keywords):
            classified["스포츠소식"].append(title)
        elif any(k in title for k in news_keywords):
            classified["스포츠뉴스"].append(title)
...
def generate_post(category, topic, date_str, img_url, photographer, is_news=False):
    img_block = ""
    if img_url:
        img_block = "\n\n![" + topic + "](" + img_url + ")\n*© " + photographer + " / Pexels*\n\n"
    ...
    if is_news:
        news_instruction = (
            "이 글은 오늘 실제 뉴스 헤드라인을 바탕으로 작성하는 글이야. "
            "헤드라인: '" + topic + "'\n"
            "이 뉴스를 바탕으로 배경 설명, 의미 분석, 향후 전망을 담아서 써줘.\n"
        )
    else:
        news_instruction = "주제: " + topic + "\n"
...
for i in range(count):
    ...
    if unique_news:
        topic = unique_news[0]
        is_news = True
        print("뉴스 기반 주제: " + topic)
    else:
        fallbacks = FALLBACK_TOPICS[category]
        unique_fallbacks = [t for t in fallbacks if not is_duplicate(t, existing)]
        topic = random.choice(unique_fallbacks if unique_fallbacks else fallbacks)
        print("기본 주제: " + topic)
...
for i, post_info in enumerate(posts_to_generate):
    ...
    img_url, photographer = get_pexels_image(topic)
    print("이미지: " + (img_url[:50] + "..." if img_url else "없음"))

    content = generate_post(category, topic, date_str, img_url, photographer, is_news)
